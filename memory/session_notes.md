...
**Last session: 2026-05-25. ⭐ TIER K IMPLEMENTATION COMPLETE (K0 + cleanup + K2 + K3 + K1 + K6+K7+K8 + K9 + K10 + K5 — 9 commits). Awaiting Sebastian's review before the `tier-k-complete` tag push. All commits live on origin/main. Render auto-deploy verified healthy (K0).**

- **K0 — `chore(deploy): K0 — expose RENDER_GIT_COMMIT on /api/health` (5877cd2).** `/api/health` now returns `{ status, timestamp, commit, env }`. Render's `RENDER_GIT_COMMIT` env var ships the SHA at build time. Polling the live URL ~3 min after push showed `commit:"5877cd2"` matching the push → **Render auto-deploy is healthy.** The earlier "old reminder copy in Build 8 QA" was almost certainly a cache/timing race, not a broken webhook. Folded a stale J2-era smoke fix (j1b case [g] still asserted the removed reminder categoryId). New smoke: `k0-health-version-test.js` (9/9).
- **Cleanup — `chore(db): Tier K cleanup — hard-delete orphan audit user` (2d06284).** Hard-deleted `weird'quote@example.com` (id `03ba3d5a-…`) from Railway via the new idempotent `scripts/cleanup-tier-k-orphans.js`. Refuses if reservations are attached or the email doesn't match. New smoke: `k-cleanup-orphans-test.js` (3/3).
- **K2 — `fix(security): K2 — helmet security headers + disable x-powered-by` (bbac063).** `app.use(helmet({ contentSecurityPolicy: false }))` + `app.disable('x-powered-by')`. Live now serves HSTS, X-Frame-Options: SAMEORIGIN, X-Content-Type-Options: nosniff, Referrer-Policy. CSP intentionally off — flagged for a later audit-driven tier. New smoke: `k2-security-headers-test.js` (6/6).
- **K3 — `fix(security): K3 — rate limit auth endpoints (login + forgot-password)` (911ec14).** `express-rate-limit` wired on `/auth/login`, `/auth/restaurant/login`, `/auth/admin/login` (5/15min per (IP, identifier) with `skipSuccessfulRequests:true`) and on both `/forgot-password` endpoints (3/hour per email + 10/hour per IP, two limiters chained). `app.set('trust proxy', 1)` so Render's X-Forwarded-For yields real client IPs. 429 returns structured `{ error: { code: 'rate-limited', retryAfterSec } }` + `Retry-After` header. Dev-only `POST /api/__test/reset-rate-limits` (404 in production) keeps the K3 smoke + downstream smokes deterministic across runs — the K1/K5/K6+K7+K8/K10 smokes all call it at startup. New smoke: `k3-rate-limit-test.js` (32/32).
- **K1 — `fix(security): K1 — stop /auth/login leaking the Prisma UserWhereInput schema` (a9b8632).** Pre-fix: empty body on `/auth/login` returned 500 with the full `UserWhereInput` shape dumped (passwordHash, expoPushToken, deletedAt, etc.) — a free schema leak. Two layers: (1) the diner `/login` handler had validators declared but never called `validationResult` → fell through to `prisma.user.findUnique({ where: { email: undefined } })`. Now it returns a clean 400 with "Email is required" / "Invalid email" / "Password is required". (2) Global error sanitizer in `index.js` — production now returns `{ error: { code: 'server-error', message: 'Something went wrong…' } }` for any 5xx or Prisma error, never echoes `err.message` or `err.stack`. Dev keeps the verbose form. New smoke: `k1-prisma-leak-test.js` (30/30).
- **K6+K7+K8 — `fix(api): K6+K7+K8 — validator hardening on reservation flow` (69d8e69).** K6: `POST /reservations` rejects past dates (Europe/Bucharest) with structured `{ error: { code: 'date-in-past' } }`. K7: `GET /restaurants/:id/time-slots` hard-caps `partySize` at 30 in the validator. K8: same endpoint adds an existence check on `restaurantId` (returns 404 `restaurant-not-found`) and the same past-date check (`date-in-past`). Mobile/restaurant clients can localize off `error.code`. Happy paths (real id + future date + party ≤30) unchanged. New smoke: `k6k7k8-validator-hardening-test.js` (12/12).
- **K9 — `fix(db): K9 — fix seed-reminder time-shift + backfill broken endTimes` (08859c1).** Audit saw rows like `time=00:29 endTime=21:00`. Initial hypothesis (`addMinutes` cross-midnight bug) was wrong — the math was correct (verified [a] in the smoke). The actual culprit: `scripts/seed-reminder-test.js` shifted `time` for the J launch-QA fixture but never updated `endTime`. Fixed the script + added `scripts/backfill-endtimes.js` (idempotent, `--dry`). Ran live; 3 rows updated; dry-run after = 0 rows. New smoke: `k9-endtime-backfill-test.js` (10/10) — addMinutes math regression, live DB has zero mismatched rows, backfill is idempotent, backfill recovers a freshly-seeded bad row.
- **K10 — `fix(api): K10 — DELETE /users/me also cancels non-terminal reservations` (69e1b2d).** Pre-K10 PII was wiped but `status` stayed PENDING/CONFIRMED/AUTO_CONFIRMED — ghost rows the restaurant couldn't action. New: second `updateMany` inside the same `$transaction` flips non-terminal rows to CANCELLED with `cancelledBy='system'` + `cancelledAt=now`. COMPLETED/NO_SHOW/already-CANCELLED rows keep their original status (historical truth). New smoke: `k10-delete-cascades-reservations-test.js` (14/14).
- **K5 — `fix(security): K5 — require password re-auth on DELETE /users/me` (a1340ef).** Stolen JWT alone can no longer wipe the account. Server: `DELETE /users/me` requires body `{ password }`, bcrypt-verified; missing → 403 `password-required`, wrong → 403 `password-incorrect`. Phone-only users (passwordHash null) deliberately get `password-incorrect` (doesn't leak whether password is unset vs wrong). Mobile: inline secure `<TextInput>` inside the existing Delete-account confirm modal; 4 new `deleteAccount.*` i18n keys (RO + EN) — passwordLabel, passwordPlaceholder, passwordRequired, passwordIncorrect. `deleteAccount()` now returns `{ ok, code }`; the screen maps the structured code to localized copy. K10 smoke updated to pass the password. **Android Metro bundle HTTP 200, 7.78 MB** ("Type your password to confirm" copy present). New smoke: `k5-delete-requires-password-test.js` (9/9).

- **Battery after every commit (final post-K5 run): all green.** C1 dispatcher 12/12, C6 popup-actions 19/19, C6 live-grid-layout 18/18, c6-assign-table-override-wiring 14/14, g-auto-confirm-picker 11/11, g-prune 5/5, g-restaurant-settings 26/26, g5b-restaurants-filter 32/32, h4-calendar-walkins 16/16, j1a-special-requests 13/13, j1b-push-token 11/11 (incl. updated [g]), j1d-reservations-order 11/11, k0-health-version 9/9, k-cleanup-orphans 3/3, k2-security-headers 6/6, k3-rate-limit 32/32, k1-prisma-leak 30/30, k6k7k8-validator-hardening 12/12, k9-endtime-backfill 10/10, k10-delete-cascades 14/14, k5-delete-requires-password 9/9, smoke-tierd2/e1/e2/f1/f2/i1/i3 all OK.

**Skipped per Sebastian's brief (not in scope this tier):** K4 (pagination — large client touch, separate session), K12 (error envelope unification — separate cleanup tier), K13/K14/K15/K16 (polish, batch later).

**Next step:** Sebastian reviews the 9 commits + Tier K behaviour, then says "tag tier-k-complete" → push `tier-k-complete` tag. After that → either K4/K12 cleanup tier, or move to the §15 backlog (the "Not built yet" features still standing — modification approval flow §5.6 polish, OOS history calendar rendering, etc.).

**Operational notes for next session:**
- The backend dev process needs `npm run dev` (nodemon) — a previous `npm start` instance was holding :4000 with NO file watcher and silently swallowed my K0 edit. Killed PID 41904 + restarted with nodemon. If a future session sees "code edits not taking effect", check whether the :4000 listener is the nodemon parent.
- The dev-only `POST /api/__test/reset-rate-limits` is gated on `NODE_ENV !== 'production'`. Live (Render) → 404. Local dev → 200. Tier K smokes call it at startup; future auth-touching smokes should too.

---

**Earlier last session: 2026-05-20. Tier J launch-fix 1d SHIPPED — three mobile-QA fix-the-fixes. Commit `fix(mobile): J1d — mobile dev IP unification + reservations ordering + pending-modification card indicator`, pushed to origin/main. Triggered by Sebastian's real-device mobile QA pass.**

- **Dev IP unification.** During QA, Cowork had hot-patched `api.js` + `socket.js` to the laptop's current LAN IP (`155.48.155.143` → `192.168.1.172`) to unblock login + sockets on the real device — those uncommitted edits are now committed. Extracted the dev host to a new single-source `apps/mobile/src/lib/devHost.js` (`DEV_LAN_IP` / `DEV_API_HOST`); `api.js` + `socket.js` both import it — a LAN-IP change is now a one-line edit. The file carries a ⚠️ comment to update it on WiFi-network change. Source-grep confirms zero `155.48.155.143` left in `apps/mobile/`. (Env-var config = v1.1.)
- **Reservations ordering.** `GET /api/reservations/mine` was `orderBy: { date: 'desc' }` → far-future bookings surfaced at the top of the diner's Rezervări tab. Fixed server-side (single source of truth): the handler now composes the canonical order — **closest upcoming first (date+time ascending), then past (most-recent first)** — partitioning on `date >= today (Bucharest) && non-terminal status`, matching the ReservationsScreen tab predicate so the tabs inherit the order directly (no client sort). New smoke `j1d-reservations-order-test.js` (11/11).
- **Pending-modification card indicator.** `ReservationsScreen` cards showed nothing when a modification was PENDING (only the rejected case had a banner). Added an informational amber banner (`pendingBanner` style — warn-tint tokens from H3, hourglass icon, no action buttons) gated on `item.modificationPending && !hasUnackRejected`; clears automatically when `modificationPending → null`. Reuses the existing `reservationDetail.pendingModNote` i18n key → identical copy to the detail screen. `ReservationDetailScreen` **already** had this indicator (`pendingNote` + `pendingModNote`, lines 218-223) — verified, no change needed there. **Zero new i18n keys.**
- **Verified:** full 19-suite battery green (C1 12/12, C6×3, g-auto-confirm/prune/settings/g5b, h4 16/16, j1a 13/13, j1b 11/11, **j1d 11/11**, smoke-tierd2/e1/e2/f1/f2/i1/i3 all OK). Android bundle clean (HTTP 200, 7.41 MB). SPEC §15 unchanged — these were QA findings, not §15-tracked items.

**⭐ TIER J IMPLEMENTATION COMPLETE (J1a → J1d).** §5.3 diner special-requests ✓, §5.7/§10 push delivery ✓ + reminder categoryId ✓, §10 SMS documented-deferred (v1.1) ✓, + the three J1d QA fix-the-fixes. **The only remaining launch gate: the 45-min reminder push end-to-end test** — needs Sebastian's physical phone (registered push token) + a Cowork-side time-adjusted reservation seed (set a confirmed reservation's start to ~45 min out and wait for the reminder cron tick) to confirm the push lands, the title/body render, and (with J1c's categoryId) the Yes/No action buttons appear and "No, cancel" cancels. Once that's green → tag MVP launch. Open v1.1 gaps (documented, not blockers): SMS transport, Calendar OOS-history rendering, dashboard notification feed.

---

**Earlier last session: 2026-05-20. Tier J launch-fix 1c SHIPPED — reminder categoryId + SMS deferral. Commit `fix(notifications): J1c — reminder categoryId on push payload + SPEC §14 SMS deferral`, pushed to origin/main. ⭐ TIER J COMPLETE END-TO-END (J1a → J1b → J1c all shipped).**
...
- **Wiring:** `AuthContext` calls `registerPushToken()` fire-and-forget on **login**, **register**, and **cold-start `checkAuth`** (cold-start catches a diner who granted permission via system Settings without re-logging-in + token rotation). `App.js` registers the category + response listener once at the root (`useEffect`, cleaned up on unmount). All failure paths (denied / offline Expo / failed POST) swallowed — never crash, never block login.
- **i18n:** 4 new `push.*` keys (RO + EN) — `actionYes`, `actionNo`, `cancelledTitle`, `cancelledBody`.
- **⚠️ Remaining (small follow-up, not a blocker):** the inline reminder **Yes/No buttons render only if the server reminder push carries `categoryId: 'reservation-reminder'`** — `channels/push.js` does NOT send it yet, so the reminder currently delivers as a tappable title/body **without** the inline buttons. Mobile category + listener are already wired; needs a ~1-line server addition (`message.categoryId` when the dispatch is the 45-min reminder). Push **delivery** — the actual launch blocker — is fixed; the buttons are §5.7 polish. Recommend folding the categoryId line into J1c or a J1d.
...
**Earlier last session: 2026-05-20. Tier J launch-fix 1a SHIPPED — diner special-requests field (§5.3). Commit `feat(mobile): J1a — diner special-requests input on BookReservationScreen + POST body field`, pushed to origin/main.**
...
**Tier J launch-fix 1b — SHIPPED (see the top of this file).**
