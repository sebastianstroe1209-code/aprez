...
**Last session: 2026-05-20. Tier G commit 6 SHIPPED — TableMove FK cascade fix (schema migration). Commit `fix(db): G6 — TableMove FK cascade fix (table → CASCADE, merged-with + reservation → SET NULL)`, pushed to origin/main.**
...
**Tier G commit 7 (next, BLOCKED on Sebastian's explicit `--accept-data-loss` approval):** the `Reservation.fromWaitlist` column drop — same migration pattern as G6 (`prisma db push --accept-data-loss`), genuinely destructive this time (a column IS dropped, though it's already a dead default-false column with no readers per the waitlist-cut decision). Plus `User.fcmToken` (deprecated, superseded by `expoPushToken`) is a candidate for the same drop — confirm with Sebastian whether G7 covers one or both. MUST NOT start without explicit go-ahead. After G7: **Tier H polish**, then **Tier J launch QA**.
