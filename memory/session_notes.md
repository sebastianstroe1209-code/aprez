...
**Last session: 2026-05-20. Tier G commit 5b SHIPPED — party/date/time availability filter on the mobile home screen (§5.1). Commit `feat(mobile): G5b — party/date/time filters on HomeScreen with per-restaurant availability join`, pushed to origin/main.**
...
**Tier G commit 6 + 7 (next, BLOCKED on Sebastian's explicit `--accept-data-loss` approval):** the two schema-migration commits — G6 + G7 drop the deprecated columns flagged across §15 (`User.fcmToken`, `Reservation.fromWaitlist`, and any other dead columns the migration audit surfaces). These need `npx prisma db push --accept-data-loss` against Railway, so they MUST NOT start without Sebastian's explicit go-ahead. After G6/G7: **Tier H polish**, then **Tier J launch QA**.
