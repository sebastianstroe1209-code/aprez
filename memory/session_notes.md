...
**Last session: 2026-05-20. Tier G commit 5a SHIPPED — Location filter on the mobile home screen (§5.1, Location only — party/date/time deferred to G5b).**
...
**⚠️ DEFERRED VERIFICATION (continues):** The DB-dependent regression battery is STILL deferred — Railway Postgres outage ongoing. G5a, like G4, changed zero backend/DB code, so deferral remains safe. The single **TODO [post-railway-recovery]** recorded in the G4 entry below now covers both G4 and G5a — a future session runs the full battery (`c6-assign-table-override-wiring`, `g-auto-confirm-picker`, `g-prune`, `g-restaurant-settings`, `smoke-tierd2/e1/e2/f1/f2/i1/i3`, + C1/C6 pure smokes already green) once the DB is back; any failure → fix-the-fix on the relevant G commit.

**Tier G commit 5b (next, waits for Sebastian's approval AND Railway recovery):** §5.1 party-size + date + time filters on the mobile home screen. Unlike G5a these REQUIRE a backend `GET /restaurants` query-param extension (today the endpoint accepts only `search`/`cuisine`/`lat`/`lng`) — so G5b has real backend changes and genuinely needs Railway Postgres reachable for an honest verification path. Do not start G5b until the DB is back.
...
**⚠️ DEFERRED VERIFICATION (post-railway-recovery TODO):** The DB-dependent regression battery was NOT run for G4 — Railway Postgres hit a major edge-network outage (incident started 2026-05-20 22:29 UTC, no ETA at commit time). Deferred suites: `c6-assign-table-override-wiring`, `g-auto-confirm-picker`, `g-prune-old-reservations`, `g-restaurant-settings`, `smoke-tierd2/e1/e2/f1/f2/i1/i3`. **Deferral is safe: G4 changed zero backend/DB code** (only restaurant-app frontend + mobile-app frontend + a Metro config) — none of those suites exercise a G4 code path. **TODO [post-railway-recovery]: a future session must run the full deferred battery once Railway Postgres is reachable; if anything fails, surface as a fix-the-fix on G4.** Quick DB-health probe: `curl -X POST localhost:4000/api/auth/restaurant/login -H 'Content-Type: application/json' -d '{"username":"lamama","password":"lamama123"}'` → 200 means DB is back.
