...
// Tier F commit 1 — Photos section for the admin restaurant edit page.
// Owns its own local state for the photos list so it can re-render
// surgically after each upload/delete/cover-toggle without forcing the
// parent's full re-fetch. Receives initial photos via prop and asks the
// parent to refresh whenever the cover changes (so Restaurant.coverPhotoUrl
// in the parent's form state stays in sync).
...
export default function PhotosSection({ restaurantId, initialPhotos = [], onChange }) {
  ...
  const handleFiles = async (e) => {
    ...
    // Client-side pre-check on count so we don't waste a round-trip per
    // file when the user shift-selects 20 photos.
    ...
    for (const file of toUpload) {
      // Pre-validate type + size client-side so we surface the friendly
      // i18n copy before going to the server.
      ...
      try {
        const created = await apiUpload(`/api/admin/restaurants/${restaurantId}/photos`, 'photo', file)
        next.push(created)
      } catch (err) {
        // Backend already returns localized error codes via api.js's
        // handleResponse → err.message. Map known codes back to copy.
  ...
  const handleSetCover = async (photoId) => {
    ...
    setWorking(photoId)
    try {
      await apiPut(`/api/admin/restaurants/${restaurantId}/photos/${photoId}/cover`, {})
  ...
  const handleDelete = async (photoId) => {
    ...
    setWorking(photoId)
    try {
      await apiDelete(`/api/admin/restaurants/${restaurantId}/photos/${photoId}`)
