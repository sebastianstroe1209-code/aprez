...
// POST /waitlist/:id/notify - Notify a waitlisted user
router.post(
  '/waitlist/:id/notify',
  authenticateRestaurant,
  [param('id').isUUID()],
...
// GET /waitlist/suggestions - Smart suggestions for fulfillment
router.get('/waitlist/suggestions', authenticateRestaurant, async (req, res, next) => {
  try {
    const prisma = req.app.get('prisma');
    const restaurantId = req.user.restaurantId;
    ...
    // Get all free tables
    const freeTables = await prisma.restaurantTable.findMany({
      where: {
        restaurantId,
        status: 'FREE',
        isActive: true,
    ...
    // Match suggestions: find entries that can fit in available tables
    const suggestions = waitlistEntries
      .map((entry) => {
        // Find a table that fits
        const suitableTable = freeTables.find((t) => t.seatCount >= entry.partySize);
        ...
        if (suitableTable) {
          return {
            ...
            suggestedTableId: suitableTable.id,
            suggestedTableNumber: suitableTable.tableNumber,
            suggestedTableSeats: suitableTable.seatCount,
          };
        }
