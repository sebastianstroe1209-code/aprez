...
- **Calendar UI doesn't show walk-ins or current-occupation state.** `TableActivity` schema model was added in C6 Phase 1; the calendar UI work is still open (consolidated below under §6.4 calendar enhancements). (The §6.4 service-period filter on the Calendar — a separate sub-item — was resolved by Tier H commit 2; see the Resolved-by-Tier-H-commit-2 entry above.)
