...
**Still open after Tier G commit 1 audit (2026-05-20):**
- **Calendar UI doesn't show walk-ins or current-occupation state.** `TableActivity` schema model was added in C6 Phase 1; the calendar UI work is still open (consolidated below under §6.4 calendar enhancements).
- **§9.3 "most free neighbors" preference missing.** When multiple exact-match tables qualify for auto-confirm, picker should prefer the one with the most adjacent free tables (for combining flexibility). Currently picks `seatCount asc` first match. Slated for Tier G commit 2 — needs a small `countFreeAdjacents()` helper extracted to `server/src/lib/tableMerges.js`.
- **§3.1 +40 phone format not validated.** Code accepts any string at registration in `auth.routes.js`. Spec requires `/^\+40\d{9}$/` regex. Slated for Tier G commit 2.
- **§3.2 30-day reservation pruning job missing.** Schema supports the rolling window query but no scheduled cleanup runs. Slated for Tier G commit 2 as a separate day-granular `setInterval` (not folded into the existing minute-loop in `socket/handlers.js`).
- **§5.1 multi-filter on mobile home missing.** Only cuisine + search exposed. Spec requires location (GPS — backend already supports `lat`/`lng` Haversine sort, UI missing), party-size, date, time as combinable filters. Party/date/time also need backend `GET /restaurants` query-param extension. Slated for Tier G commit 5 (likely split into 5a Location + 5b party/date/time).
- **§5.2 mobile profile gaps.** Cover photo + swipeable carousel + menu-PDF `Linking.openURL` all shipped in F1. **Still missing: Google Maps embed** (`Restaurant.latitude` + `longitude` are in the schema + GET payload, just unused — needs `react-native-maps` install + a `<MapView>` mount). Slated for Tier G commit 4.
- **`PUT /api/restaurant/settings` endpoint missing (silent-bug today).** `apps/restaurant/app/dashboard/settings/page.js:96` posts to this route, but no handler is mounted in `restaurantPlatform.routes.js`. The auto-confirm toggle and any other Settings save fails silently with no error toast. Slated for Tier G commit 3 alongside §6.7 / §7.6.
